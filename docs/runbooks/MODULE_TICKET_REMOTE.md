...
For the version 3 non-destructive ViVi account switch, preserve the compatibility order. First deploy a Pixel that accepts versions 1, 2, and 3 while retaining the prior version 1 semantics. Next publish the additive `ticketremote_owner_request_vivi_reauth_logout_login` reducer. Last, deploy the rebuilt admin page and generated browser/sidecar bindings that begin producing version 3. Version 3 uses a `vivi-logout-login-` request ID and the exact private command flag `logoutInApp: true`; version 2 full reset remains separate with `resetAppData: true`. To roll back, remove or disable the version 3 browser producer first, wait until no matching queued, pending, or running attempt remains, and downgrade Pixel last. Never reinterpret version 1, automatically fall back to full reset, or downgrade Pixel while a version 3 command can still run.
