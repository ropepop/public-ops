...
Immediate signed-in Ticket controls use the member `ticket_action_v3` reducer directly. The warm Pixel path is the pending-command subscription; the poller is reconnect/acknowledgement recovery. The browser requests non-mutating `prove_current` on the first fresh frame of each stream epoch, after two agreeing significant picture changes, and on visible resume only when the prior current-view proof is stale. A busy phone defers rather than discards a pending picture-change trigger, and an unknown result is retried only after another meaningful change.
