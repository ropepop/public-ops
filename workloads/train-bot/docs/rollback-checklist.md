...
2. Run `../../tools/arbuzas/deploy.sh rollback --release-id "<previous-release-id>" --ssh-host kitty-gration --ssh-user "$USER"`.
3. Re-run `../../tools/arbuzas/deploy.sh validate --release-id "<previous-release-id>" --ssh-host kitty-gration --ssh-user "$USER"`.
