...
- Every active authenticated member may opt into the downstream HDR image view. The account-scoped database preference defaults off and follows the account across browsers and devices; the original SDR stream stays live underneath and immediately takes over on unsupported capability, stale input, socket failure, invalid output, or sustained transform failure. Healthy HDR replacements decode before an atomic surface swap, so a new conversion never deliberately reveals SDR.
- The local registration drag may start anywhere. It still requires deliberate held rightward movement and completes only after either 25% of the slider width or the native terminal end; a terminal tap, short non-terminal drag, reverse drag, cancellation, second pointer, scrolling, or stale proof submits nothing.
