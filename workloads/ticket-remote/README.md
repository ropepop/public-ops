...
- Pixel keeps one subscription for every pending Ticket command and one dispatch ledger shared with reconnect polling. The subscription is the warm path; polling is catch-up and acknowledgement recovery. An uncertain physical tap or registration drag is visually reconciled and never blindly repeated.
