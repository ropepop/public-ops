...
func TestBrowserSocketConnectionLimits(t *testing.T) {
	t.Run("session", func(t *testing.T) {
		...
		for i := 0; i < maxBrowserSocketsPerSession; i++ {
			candidate := &client{sessionID: "session-a", email: "member@example.test", video: true}
		...
		if server.tryAddClient(&client{sessionID: "session-a", email: "member@example.test", video: true}) {
	...
	t.Run("identity", func(t *testing.T) {
		...
		for i := 0; i < maxBrowserSocketsPerIdentity; i++ {
			candidate := &client{sessionID: fmt.Sprintf("session-%d", i), email: "MEMBER@example.test", video: true}
			if !server.tryAddClient(candidate) {
				t.Fatalf("connection %d within the per-identity limit was rejected", i+1)
			}
		}
		if server.tryAddClient(&client{sessionID: "one-more-session", email: "member@example.test", video: true}) {
	...
	t.Run("server", func(t *testing.T) {
		...
		if server.tryAddClient(&client{sessionID: "overflow", email: "overflow@example.test", video: true}) {
