...
func TestAdminTicketReselectLatestLegacyProducerIsRetired(t *testing.T) {
	activeFile := filepath.Join(t.TempDir(), "active-phone-backend.json")
	...
	if rec.Code != http.StatusGone || !strings.Contains(rec.Body.String(), `"error":"route_retired"`) {
		t.Fatalf("retired reselect status = %d body = %s", rec.Code, rec.Body.String())
	}
	if len(store.commands) != 0 {
		t.Fatalf("retired reselect queued commands = %#v", store.commands)
	}
}
