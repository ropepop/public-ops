...
type Broker struct {
	...
	wake                     chan struct{}
	rsLogin                  *RSLoginJob
	rsLoginControl           *websocket.Conn
	rsLoginCancel            context.CancelFunc
	rsLoginAttempts          int
	rsLoginSuccesses         int
	rsLoginFailures          int
	rsLoginFailureByReason   map[string]int
	rsLoginLastAt            time.Time
	rsLoginLastState         string
	rsLoginLastFailureReason string
	rsLoginLastPhoneLast4    string
}
...
type StateSnapshot struct {
	...
	RSLogin              RSLoginPublicSnapshot `json:"rsLogin"`
}
...
type AnalyticsSnapshot struct {
	...
	RSLogin     RSLoginAnalytics `json:"rsLogin"`
}
...
func (b *Broker) Handler() http.Handler {
	...
	mux.HandleFunc("/api/v1/rs/login", b.handleRSLoginStatus)
	mux.HandleFunc("/api/v1/rs/login/start", b.handleRSLoginStart)
	mux.HandleFunc("/api/v1/rs/login/sms", b.handleRSLoginSMS)
	mux.HandleFunc("/api/v1/rs/login/cancel", b.handleRSLoginCancel)
...
func (b *Broker) UpdateTicketPresence(ctx context.Context, input TicketPresenceInput) error {
	...
	if input.Viewers > 0 {
		...
		if b.ticketBlocksQRLocked(now) {
			...
			b.preemptRSLoginForTicketLocked(now)
		}
...
func (b *Broker) AcquireTicketLease(ctx context.Context, input TicketLeaseInput) (TicketLeaseSnapshot, error) {
	...
	if b.ticketBlocksRSLoginLocked(now) {
		b.preemptRSLoginForTicketLocked(now)
	}
...
func (b *Broker) Snapshot(now time.Time) StateSnapshot {
	...
	return StateSnapshot{
		...
		Jobs:                 jobs,
		RSLogin:              b.publicRSLoginSnapshotLocked(),
	}
}
...
func (b *Broker) Analytics(now time.Time) AnalyticsSnapshot {
	...
	b.mu.Lock()
	loginAnalytics := RSLoginAnalytics{
		Attempts:          b.rsLoginAttempts,
		Successes:         b.rsLoginSuccesses,
		Failures:          b.rsLoginFailures,
		FailureByReason:   cloneStringIntMap(b.rsLoginFailureByReason),
		LastState:         b.rsLoginLastState,
		LastFailureReason: b.rsLoginLastFailureReason,
		LastPhoneLast4:    b.rsLoginLastPhoneLast4,
	}
	if !b.rsLoginLastAt.IsZero() {
		loginAnalytics.LastAt = b.rsLoginLastAt.UTC().Format(time.RFC3339Nano)
	}
	liveLogin := b.publicRSLoginSnapshotLocked()
	b.mu.Unlock()
	if loginAnalytics.LastState == "" {
		loginAnalytics.LastState = liveLogin.State
	}
	if loginAnalytics.LastPhoneLast4 == "" {
		loginAnalytics.LastPhoneLast4 = liveLogin.PhoneLast4
	}
	if loginAnalytics.LastAt == "" && liveLogin.StartedAt != "" {
		loginAnalytics.LastAt = liveLogin.StartedAt
	}
	analytics.RSLogin = loginAnalytics
	return analytics
}
...
func (b *Broker) handleRSLoginStart(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	...
	snapshot, err := b.StartRSLogin(r.Context(), req.Phone, time.Now())
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"ok": false, "error": err.Error()})
		return
	}
	writeJSON(w, http.StatusAccepted, map[string]any{"ok": true, "rsLogin": snapshot})
}

func (b *Broker) handleRSLoginSMS(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	...
	snapshot, err := b.SubmitRSLoginSMS(r.Context(), req.Code, time.Now())
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"ok": false, "error": err.Error()})
		return
	}
...
func (b *Broker) handleRSLoginCancel(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	snapshot, err := b.CancelRSLogin(r.Context(), time.Now())
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"ok": false, "error": err.Error()})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "rsLogin": snapshot})
}

func (b *Broker) handleRSLoginStatus(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "rsLogin": b.RSLoginSnapshot()})
}
