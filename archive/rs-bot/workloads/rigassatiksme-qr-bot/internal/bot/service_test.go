...
func TestCleanReasonMapsRsAuthBlockedToFriendlyLatvianText(t *testing.T) {
	got := cleanReasonForLanguage("rs_auth_blocked", "lv")
	want := "Rīgas Satiksme lietotnei beigusies sesija telefonā. Lūdzu, mēģini vēlreiz pēc brīža — ja neatjaunojas, admins var atjaunot sesiju ar /admin login_rs."
	if got != want {
		t.Fatalf("cleanReasonForLanguage(rs_auth_blocked, lv) = %q, want %q", got, want)
	}
	if strings.Contains(got, "auth_blocked") || strings.Contains(got, "auth blocked") {
		t.Fatalf("latvian text leaked raw reason: %q", got)
	}
}

func TestCleanReasonMapsRsAuthBlockedToFriendlyRussianText(t *testing.T) {
	got := cleanReasonForLanguage("rs_auth_blocked", "ru")
	want := "Сессия приложения Rīgas Satiksme истекла на телефоне. Попробуй позже — если не получится, админ может восстановить сессию через /admin login_rs."
	if got != want {
		t.Fatalf("cleanReasonForLanguage(rs_auth_blocked, ru) = %q, want %q", got, want)
	}
	if strings.Contains(got, "auth_blocked") || strings.Contains(got, "auth blocked") {
		t.Fatalf("russian text leaked raw reason: %q", got)
	}
}

func TestCleanReasonMapsRsAuthBlockedToFriendlyEnglishText(t *testing.T) {
	if got := cleanReason("rs_auth_blocked"); got != "Rīgas Satiksme app session expired on the phone. Please try again in a moment — if it does not recover, an admin can restore the session with /admin login_rs." {
		t.Fatalf("cleanReason(rs_auth_blocked) = %q", got)
	}
	if strings.Contains(cleanReason("rs_auth_blocked"), "auth_blocked") {
		t.Fatalf("english text leaked raw reason")
	}
}

func TestCleanReasonUnknownFallsThroughUnchanged(t *testing.T) {
	if got := cleanReason("rs_brand_new_reason"); got != "rs brand new reason" {
		t.Fatalf("cleanReason(rs_brand_new_reason) = %q, want %q", got, "rs brand new reason")
	}
}
...
func TestAdminLoginRSStartRequiresAdmin(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, &fakeBroker{})

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: 42, ChatType: "private", UserID: 42, Text: "/admin login_rs +371 20000000"}); err != nil {
		t.Fatal(err)
	}
	if !telegram.hasMessageContaining("Tikai administratoram") {
		t.Fatalf("non-admin login_rs should get admin-only response: %#v", telegram.messages)
	}
}

func TestAdminLoginRSStartRequiresPhone(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, &fakeBroker{})

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs"}); err != nil {
		t.Fatal(err)
	}
	if !telegram.hasMessageContaining("Usage:") {
		t.Fatalf("login_rs without phone should show usage: %#v", telegram.messages)
	}
}

func TestAdminLoginRSStartCallsBroker(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginSnapshot: RSLoginSnapshot{State: "waiting_for_sms", PhoneLast4: "0000"}}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs +371 20000000"}); err != nil {
		t.Fatal(err)
	}
	if broker.rsLoginStartCount != 1 {
		t.Fatalf("start count = %d, want 1", broker.rsLoginStartCount)
	}
	if broker.rsLoginLastPhone != "+371 20000000" {
		t.Fatalf("last phone = %q, want +371 20000000", broker.rsLoginLastPhone)
	}
	if !telegram.hasMessageContaining("0000") {
		t.Fatalf("phone last 4 not in confirmation: %#v", telegram.messages)
	}
}

func TestAdminLoginRSStartRequiresPrivateChat(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginSnapshot: RSLoginSnapshot{State: "waiting_for_sms", PhoneLast4: "0000"}}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: -100, ChatType: "group", UserID: 7, Text: "/admin login_rs +371 20000000"}); err != nil {
		t.Fatal(err)
	}
	if broker.rsLoginStartCount != 0 {
		t.Fatalf("start count = %d, want 0 (group chat blocked)", broker.rsLoginStartCount)
	}
	if !telegram.hasMessageContaining("private") {
		t.Fatalf("expected private-chat-only message: %#v", telegram.messages)
	}
}

func TestAdminLoginRSSMSRequiresActiveLogin(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, &fakeBroker{})

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs_sms 12345"}); err != nil {
		t.Fatal(err)
	}
	if !telegram.hasMessageContaining("No active RS login") {
		t.Fatalf("expected no-active-login message: %#v", telegram.messages)
	}
}

func TestAdminLoginRSSMSDispatchesAndNeverEchoesCode(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginSnapshot: RSLoginSnapshot{State: "waiting_for_sms", PhoneLast4: "0000"}}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)
	...
	if err := service.HandleMessage(context.Background(), Message{MessageID: 2, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs_sms 12345"}); err != nil {
		t.Fatal(err)
	}
	if broker.rsLoginSMSCount != 1 {
		t.Fatalf("sms count = %d, want 1", broker.rsLoginSMSCount)
	}
	if broker.rsLoginLastCode != "12345" {
		t.Fatalf("last sms code = %q, want 12345", broker.rsLoginLastCode)
	}
	for _, msg := range telegram.messages {
		if strings.Contains(msg, "12345") {
			t.Fatalf("bot reply leaked SMS code: %q", msg)
		}
	}
}

func TestAdminLoginRSStatusDelegatesToBroker(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginSnapshot: RSLoginSnapshot{State: "waiting_for_sms", PhoneLast4: "0000"}}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)
	...
	if err := service.HandleMessage(context.Background(), Message{MessageID: 2, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs_status"}); err != nil {
		t.Fatal(err)
	}
	if !telegram.hasMessageContaining("0000") {
		t.Fatalf("expected status to mention last 4: %#v", telegram.messages)
	}
}

func TestAdminLoginRSCancelDelegatesToBroker(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginSnapshot: RSLoginSnapshot{State: "waiting_for_sms", PhoneLast4: "0000"}}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)
	...
	if err := service.HandleMessage(context.Background(), Message{MessageID: 2, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs_cancel"}); err != nil {
		t.Fatal(err)
	}
	if err := service.HandleMessage(context.Background(), Message{MessageID: 3, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs_sms 12345"}); err != nil {
		t.Fatal(err)
	}
	if broker.rsLoginSMSCount != 0 {
		t.Fatalf("sms count after cancel = %d, want 0 (cancel must clear in-flight state)", broker.rsLoginSMSCount)
	}
}

func TestAdminLoginRSStartSurfacedFailure(t *testing.T) {
	access := NewMemoryAccessManager(AccessConfig{AdminUserIDs: []string{"7"}, DefaultOpen: false, Clock: fixedAccessClock(t)})
	broker := &fakeBroker{rsLoginStartErr: errors.New("phone must contain between 6 and 16 digits")}
	telegram := &fakeTelegram{}
	service := NewService(ServiceConfig{Access: access}, telegram, broker)

	if err := service.HandleMessage(context.Background(), Message{MessageID: 1, ChatID: 7, ChatType: "private", UserID: 7, Text: "/admin login_rs 123"}); err != nil {
		t.Fatal(err)
	}
	if !telegram.hasMessageContaining("phone must contain between 6 and 16 digits") {
		t.Fatalf("expected broker error in bot reply: %#v", telegram.messages)
	}
}
...
type fakeBroker struct {
	...
	rsLoginSnapshot   RSLoginSnapshot
	rsLoginStartErr   error
	rsLoginSMSErr     error
	rsLoginCancelErr  error
	rsLoginStatusErr  error
	rsLoginStartCount int
	rsLoginSMSCount   int
	rsLoginLastPhone  string
	rsLoginLastCode   string
}
...
func (b *fakeBroker) StartRSLogin(ctx context.Context, phone string) (RSLoginSnapshot, error) {
	b.rsLoginStartCount++
	b.rsLoginLastPhone = phone
	if b.rsLoginStartErr != nil {
		return RSLoginSnapshot{}, b.rsLoginStartErr
	}
	snapshot := b.rsLoginSnapshot
	if snapshot.PhoneLast4 == "" {
		snapshot.PhoneLast4 = "0000"
	}
	return snapshot, nil
}

func (b *fakeBroker) SubmitRSLoginSMS(ctx context.Context, code string) (RSLoginSnapshot, error) {
	b.rsLoginSMSCount++
	b.rsLoginLastCode = code
	if b.rsLoginSMSErr != nil {
		return RSLoginSnapshot{}, b.rsLoginSMSErr
	}
	return b.rsLoginSnapshot, nil
}

func (b *fakeBroker) CancelRSLogin(ctx context.Context) (RSLoginSnapshot, error) {
	if b.rsLoginCancelErr != nil {
		return RSLoginSnapshot{}, b.rsLoginCancelErr
	}
	return b.rsLoginSnapshot, nil
}

func (b *fakeBroker) RSLoginStatus(ctx context.Context) (RSLoginSnapshot, error) {
	if b.rsLoginStatusErr != nil {
		return RSLoginSnapshot{}, b.rsLoginStatusErr
	}
	return b.rsLoginSnapshot, nil
}
