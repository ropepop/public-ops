...
- RS monthly-ticket reasons must preserve Pixel's named phone result when one exists. `phone_timeout` means no final Pixel outcome arrived before the broker deadline; it must not replace reasons such as `rs_phone_automation_unavailable`, `rs_app_launch_failed`, `rs_app_foreground_failed`, `wrong_code`, `code_rejected_by_rs`, `rs_monthly_ticket_missing`, `rs_manual_code_field_missing`, `rs_manual_code_button_missing`, `rs_monthly_ticket_stale_code`, `rs_monthly_ticket_unknown_state`, `rs_monthly_ticket_state_timeout`, `rs_monthly_ticket_image_capture_failed`, or `rs_auth_blocked`. Non-critical cleanup may be delayed briefly after a final result so rapidly arriving RS work can stay inside the warm RS app.

## RS re-login channel

The Rīgas Satiksme passenger app on the Pixel has its own session lifecycle that is independent of ViVi. When the RS app's session expires, the Pixel semantic driver matches the dialog text ("Your session has expired. Please log in again.") and returns `AUTH_BLOCKED`, which the broker surfaces as the named final failure `rs_auth_blocked`. The broker does not retry it.

To recover without touching the phone, the Telegram `rs biļete` bot exposes an admin-driven re-login channel. The bot is the only safe UI surface for the SMS code; the broker never logs the code and only retains the last 4 digits of the phone number for human inspection.

- **Broker commands** to the Pixel: `rigassatiksme_login_start { requestId, phone, locale, serverSentAt }`, `rigassatiksme_login_sms { requestId, code, serverSentAt }`, `cancel_rigassatiksme_login { requestId, reason }`.
- **Broker result** from the Pixel: `rigassatiksme_login_result { requestId, state, failureReason? }` where `state` is one of `waiting_for_sms | succeeded | failed`.
- **Broker HTTP surface**: `POST /api/v1/rs/login/start`, `POST /api/v1/rs/login/sms`, `POST /api/v1/rs/login/cancel`, `GET /api/v1/rs/login`.
- **Broker `/api/v1/state`** now includes a safe `rsLogin` field with `state`, `phoneLast4`, `startedAt`, `completedAt`, `failureReason`, and `smsAttempts`. The full phone number and the SMS code are never written to broker state, never logged, and never exposed in any HTTP response.
- **Broker `/api/v1/analytics`** now includes an `rsLogin` rollup: `attempts`, `successes`, `failures`, `failureByReason`, `lastState`, `lastFailureReason`, `lastPhoneLast4`, `lastAt`.
- **Policy**: exactly one SMS attempt per `start`. On `login_failed { failureReason: wrong_sms_code }` the broker finalizes the login as failed and waits for a fresh `login_start` from the admin; there is no automatic resend.
- **Serialization**: a new login preempts a running RS QR job (`login_preempted`) and is itself preempted by ticket presence (`ticket_preempted`) exactly the same way a QR job would be. RS QR jobs and login jobs share the phone but never run simultaneously.
- **Failure reasons** for the login channel are: `phone_field_missing`, `sms_field_missing`, `wrong_sms_code`, `sms_timeout`, `submit_failed`, `phone_unavailable`, `ticket_preempted`, `canceled`, `login_unreachable`.
- **Bot commands**: `/admin login_rs <phone>` (start), `/admin login_rs_sms <code>` (type SMS), `/admin login_rs_status` (snapshot), `/admin login_rs_cancel` (abort). Admin-only, private-chat-only. The bot logs only the request id and the redacted `last4`; SMS codes are zeroed in broker memory right after dispatch and are never written to any log, state file, or HTTP response.
...
- 2026-06-16: `rs_auth_blocked` is now a recognized broker reason with a user-facing friendly translation in the bot (LV / RU / EN) that points admins at the new `/admin login_rs` re-login channel. The Pixel semantic driver was already matching the "session expired" dialog; the new behavior is the bot side, the broker admin channel, and the safety rules around the SMS code.
- 2026-06-16: Added a broker-side admin-driven RS re-login channel that allows an admin to type a phone number and an SMS confirmation code in a private Telegram chat. The Pixel receives `rigassatiksme_login_start` and `rigassatiksme_login_sms` commands and reports a `rigassatiksme_login_result` back. The broker keeps a safe `rsLogin` snapshot (last 4 digits only) in `/api/v1/state` and a rollup in `/api/v1/analytics`. SMS codes are never persisted, logged, or echoed; the phone is persisted only as last-4. One SMS attempt per `start`; wrong code means a fresh `start`.
