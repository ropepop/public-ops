...
MANAGED_COMMENT = "# Analyzer credentials are stored in root-only host secret files."
...
def resolved_values(assignments: dict[str, str]) -> dict[str, str]:
    ...
    if not values["SATIKSME_CHAT_ANALYZER_GOOGLE_API_KEY"]:
        values["SATIKSME_CHAT_ANALYZER_GOOGLE_API_KEY"] = assignments.get(
            "SATIKSME_CHAT_ANALYZER_MODEL_API_KEY", ""
        ).strip()
...
def validate(
    ...
    retired = sorted(key for key in RETIRED_MODEL_KEYS if key in assignments)
    if retired:
        raise SystemExit("retired analyzer model-key settings remain: " + ", ".join(retired))
...
def rewrite_managed_assignments(
    env_file: Path,
    original_lines: list[str],
    runtime_secret_root: str,
) -> None:
    managed_keys = set(DIRECT_KEYS) | set(FILE_KEYS.values()) | RETIRED_MODEL_KEYS
    retained = []
    for raw in original_lines:
        key = raw.split("=", 1)[0].strip() if "=" in raw else ""
        if key not in managed_keys and raw.strip() != MANAGED_COMMENT:
            retained.append(raw)
...
def migrate(env_file: Path, secrets_dir: Path, runtime_secret_root: str) -> None:
    ...
    if not any(assignments.get(key, "").strip() for key in DIRECT_KEYS):
        if any(key in assignments for key in RETIRED_MODEL_KEYS):
            rewrite_managed_assignments(env_file, original_lines, runtime_secret_root)
...
def set_google_key_from_stdin(secrets_dir: Path) -> None:
    ...
    target = secrets_dir / "google-api-key.secret"
...
def main() -> int:
    ...
    if args.set_google_key_stdin:
        ...
        print("Satiksme analyzer Google credential replaced in its private file")
        return 0
