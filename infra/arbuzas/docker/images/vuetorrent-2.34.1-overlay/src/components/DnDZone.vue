...
import { materializeDroppedFiles } from '@/utils/materializeDroppedFiles'
...
function showAddError(torrentsCount: number) {
  toast.error(t('toast.add.error', torrentsCount), { autoClose: 1500 })
}

async function materializeTorrentFiles(torrentFiles: File[], torrentsCount: number): Promise<File[] | undefined> {
  if (!torrentFiles.length) return torrentFiles

  try {
    return await materializeDroppedFiles(torrentFiles)
  } catch {
    showAddError(torrentsCount)
  }
}
...
async function onQueueDrop(files: File[] | null, event: DragEvent) {
  ...
  if (!torrentsCount) return

  const materializedFiles = await materializeTorrentFiles(torrentFiles, torrentsCount)
  if (!materializedFiles) return

  materializedFiles.forEach(addTorrentStore.pushTorrentToQueue)
...
async function onDownloadDrop(files: File[] | null, event: DragEvent) {
  if (!checkDropEvent(event)) return
  ...
  return toast.promise(
    torrentStore.addTorrents(materializedFiles, links),
    {
