...
RUN --mount=type=cache,target=/go/pkg/mod \
  ...
  CGO_ENABLED=0 GOOS="${TARGETOS:-linux}" GOARCH="${TARGETARCH:-amd64}" \
    go build -ldflags "$ldflags" -o /out/chatgpt-bot ./cmd/chatgpt-bot
...
WORKDIR /srv/chatgpt-bot

COPY --from=build /out/chatgpt-bot /usr/local/bin/chatgpt-bot

CMD ["/usr/local/bin/chatgpt-bot"]
